...
PALWORLD_VERSION := $(shell awk '/^version:/{print $$2}' charts/palworld/Chart.yaml)

lint: lint-actions-runner lint-deemix lint-filebrowser lint-gha-runner-scale-set lint-hercules-ci-agent lint-mage-server lint-palworld
...
package: .cr-release-packages/actions-runner-$(ACTIONS_RUNNER_VERSION).tgz \
	...
	.cr-release-packages/palworld-$(PALWORLD_VERSION).tgz
...
.cr-release-packages/palworld-$(PALWORLD_VERSION).tgz: CHART := palworld
